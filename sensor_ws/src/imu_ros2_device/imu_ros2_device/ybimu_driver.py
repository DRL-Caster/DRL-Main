...
class ybimu_driver(Node):
    ...
    def pub_data(self):
        ...
        # self.get_logger().info("ax = {}, ay = {}, az = {} ".format(ax,ay,az))  # 调试用：打印加速度计原始值
        [gx, gy, gz] = self.robot.get_gyroscope_data()
        # self.get_logger().info("gx = {}, gy = {}, gz = {} ".format(gx,gy,gz))  # 调试用：打印陀螺仪原始值
        [mx, my, mz] = self.robot.get_magnetometer_data()
        # self.get_logger().info("mx = {}, my = {}, mz = {} ".format(mx,my,mz))  # 调试用：打印磁力计原始值
